...
const PaymentRequestForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                toast.success(data.message || 'Payment request submitted successfully! 🎉'); // Using toast
                setAmount('');
                setSelectedBankAccount('');
                fetchWalletBalance();
            } else {
                setError(data.message || 'Failed to submit payment request. ❌');
                toast.error(data.message || 'Failed to submit payment request. 😥'); // Using toast
            }
        } catch (err) {
            console.error('Error submitting payment request:', err);
            setError('An unexpected error occurred while submitting the request. ⚠️');
            toast.error('An unexpected error occurred while submitting the request. 🤕'); // Using toast
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {isLoadingBalance ? (
                <p className="text-sm text-gray-500">Fetching wallet balance... ⏳</p>
            ) : errorBalance ? (
                <p className="text-sm text-red-500">{errorBalance}</p>
            ) : walletBalance !== null && (
                <p className="text-sm text-gray-600">
                    Your current wallet balance: ₦{walletBalance.toLocaleString()}
                </p>
            )}
            <div>
