    ...
    try {
        ...
        if (status && status !== 'all') {
            ...
            filteredTransactionHistory = filteredTransactionHistory.filter(tx => {
                const transactionStatus = tx.status ? tx.status.toLowerCase().trim().replace(/ /g, '-') : null;
                const matches = transactionStatus === lowerStatus;
                return matches;
            });
        }

        filteredTransactionHistory.sort((a, b) => b.createdAt - a.createdAt);


        res.status(200).json({ success: true, transactions: filteredTransactionHistory });
